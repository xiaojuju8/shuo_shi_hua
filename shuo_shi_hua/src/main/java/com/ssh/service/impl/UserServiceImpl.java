...
import com.ssh.utils.RegexUtils;
...
import static com.ssh.utils.SystemConstants.USER_NICK_NAME_PREFIX;
...
public  class UserServiceImpl extends ServiceImpl<UserMapper, User> implements IUserService {
    ...
    public Result sendCode(String phone, HttpSession session) {
        ...
        //4.保存校验码到redis,设置有效时间为10分钟
        stringRedisTemplate.opsForValue().set(LOGIN_CODE_KEY+phone,code,LOGIN_CODE_TTL, TimeUnit.MINUTES);

        //5.发送校验码
        log.debug("发送短信校验码成功，校验码：{}",code);
        //返回ok
        return Result.ok();
    }
    ...
    public Result signCount() {
        ...
        //4.获取今天是本月第几天
        int dayOfMonth = now.getDayOfMonth();
        //5.获取本月截止到今天的所有签到记录，返回一个十进制数字 BITFIELD sign:2:202403 GET U23 0
